      ...
      <form onSubmit={handleGenerate} className="max-w-4xl mx-auto mb-40">
        ...
        <div className="flex gap-20 max-sm:flex-col items-start justify-between">
          ...
          <div className="w-full ">
            ...
            <div className="mb-4 text-gray-300">
              ...
              <input
                type="text" id="productName" value={productName} onChange={(e) => setName(e.target.value)}
                placeholder="Enter the name of the product" required
                className="w-full bg-white/5 rounded-lg border-2 p-4 text-sm border-violet-200/10 focus:border-violet-500/50 outline-none transition-all"/>
            </div>

            <div className="mb-4 text-gray-300">
              <label htmlFor="productDescription" className="block text-sm mb-4">Product Description <span className="text-xs text-voilet-400">(optional)</span></label>
              <textarea name="productDescription" rows={4} value={productDescription} onChange={(e) => setProductDescription(e.target.value)} placeholder="Enter the description of the product" className="w-full bg-white/3 rounded-lg border-2 p-4 text-sm border-violet-200/10 focus:border-violet-500/50 outline-none resize-none transition-all"/>
            </div>
            ...
            <div className="mb-4 text-gray-300">
              <label htmlFor="UserPrompt" className="block text-sm mb-4">User Prompt <span className="text-xs text-voilet-400">(optional)</span></label>
        ...
        <div className="flex justify-center mt-10">
          <PrimaryButton disabled={isGenerating} className="px-10 py-3 rounded-md disabled:opacity-70 disabled:cursor-not-allowed">
            {isGenerating ? (
              <>
              <Loader2Icon className="size-5 animation-spin"/> Generating...
              </>
            ):(
            <>
